...
ByteAddressBuffer _319 : register(t1);
RWByteAddressBuffer _331 : register(u2);
cbuffer EngineGlobals_visibility_occlusion_test_comp_hlsl : register(b11)
{
    int uCandidateCount;
...
Texture2D<float4> uHiZ : register(t0);
SamplerState _uHiZ_sampler : register(s0);
...
bool hizBoundsVisible(float4x4 viewProjection, float3 boundsMin, float3 boundsMax, Texture2D<float4> hiZ, SamplerState _hiZ_sampler, float depthBias, int maxMip)
{
    ...
    for (int corner = 0; corner < 8; corner++)
    {
        ...
        uvMin = min(uvMin, uv);
        uvMax = max(uvMax, uv);
        objectNear = min(objectNear, (ndc.z * 0.5f) + 0.5f);
    }
    bool _140 = uvMax.x <= 0.0f;
    bool _147;
    if (!_140)
    {
        _147 = uvMax.y <= 0.0f;
    ...
    float2 fullSize = float2(int2(spvTextureSize(hiZ, uint(0), _178_dummy_parameter)));
    ...
    uint _211_dummy_parameter;
    int2 mipSize = int2(spvTextureSize(hiZ, uint(mip), _211_dummy_parameter));
...
void comp_main()
{
    ...
    Bounds _323;
    _323.Minimum = asfloat(_319.Load4(index * 32 + 0));
    _323.Maximum = asfloat(_319.Load4(index * 32 + 16));
    Bounds bounds;
    bounds.Minimum = _323.Minimum;
    bounds.Maximum = _323.Maximum;
    float4x4 param = uViewProjection;
    float3 param_1 = bounds.Minimum.xyz;
    float3 param_2 = bounds.Maximum.xyz;
    float param_3 = uDepthBias;
    int param_4 = uMaxMip;
    _331.Store(index * 4 + 0, uint(hizBoundsVisible(param, param_1, param_2, uHiZ, _uHiZ_sampler, param_3, param_4)));
}
